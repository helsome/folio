    ...
    <div className={`rounded-[10px] border bg-surface-raised p-3.5 transition-colors ${rule.enabled ? 'border-border' : 'border-border/70 bg-surface-muted/55'}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
            ...
            <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${rule.enabled ? 'bg-positive/10 text-positive' : 'bg-foreground/7 text-foreground/48'}`}>
              {rule.enabled ? t('automation.status.active') : t('automation.status.paused')}
            </span>
          </div>
          <div className="mt-1 text-[12px] text-foreground/54">{scheduleSummary(rule, t)}</div>
        </div>
          ...
          <span
            className={`rounded-full px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide ${
              rule.notify === 'material-only'
                ...
                : 'bg-foreground/7 text-foreground/54'
            }`}
          >
            {rule.notify === 'material-only'
      ...
      <div className="mt-3 flex items-center justify-between gap-3 border-t border-border/70 pt-2.5">
        ...
        <button
          ...
          className="rounded-[7px] bg-primary px-2.5 py-1.5 text-[11px] font-medium text-primary-foreground transition-colors hover:bg-primary/88 disabled:cursor-not-allowed disabled:opacity-45 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        >
