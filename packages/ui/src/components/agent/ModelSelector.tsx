...
export const ModelSelector: React.FC<ModelSelectorProps> = ({ disabled = false }) => {
  ...
  const currentLabel = state.loading ? 'Loading models…' : current ? current.name || `${current.provider}/${current.id}` : 'Select model';
  ...
  return <div className="min-w-0">
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        <button type="button" disabled={disabled || busy} aria-label="Model" className="flex max-w-[190px] items-center gap-1.5 rounded-[8px] border border-border bg-surface px-2.5 py-1.5 text-[12px] font-medium text-foreground/80 transition-colors hover:bg-surface-hover disabled:pointer-events-none disabled:opacity-45">
      ...
      <DropdownMenuContent align="start" className="max-h-72 w-64 overflow-y-auto">
        {groups.length === 0 && !state.loading ? <div className="px-3 py-4 text-center text-[12px] text-foreground/42">No models available</div> : groups.map((group) => <React.Fragment key={group.provider}>
