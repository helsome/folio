/**
 * Output types for the phase-2 Longbridge fetchers.
 *
...
/** A raw equity position returned by the `positions` command. */
...
/** Per-currency cash detail inside the account assets snapshot. */
...
/** One cash-flow record (deposit, withdrawal, dividend, settlement, …). */
export interface CashFlowRecord {
  /** Epoch seconds of the flow. */
  timestamp: number;
  balance: number;
  businessType: string;
  flowName: string;
